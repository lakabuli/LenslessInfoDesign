#!/bin/bash
# Script to download the DiffuserCam model from Google Drive
# Downloads the model from reconstruction/experiments_diffusercam in the shared folder
...
# Find and copy the experiments_diffusercam folder contents to current directory
if [ -d "${TEMP_DIR}/reconstruction/experiments_diffusercam" ]; then
    echo "Copying model files to current directory..."
    cp -r "${TEMP_DIR}/reconstruction/experiments_diffusercam"/* .
    echo "Download complete! Model files are in the current directory."
else
    echo "Warning: Could not find reconstruction/experiments_diffusercam in downloaded folder."
