...
# Script for running reconstruction sweeps on ideal PSF designs (random and heuristic initialization).
# Evaluates reconstruction models across multiple datasets (MNIST, Fashion MNIST, CIFAR10),
# learning rates, and random seeds.
...
ideal_cifar = np.load(ideal_dir + "ideal_cifar_psf.npy")
ideal_fashion = np.load(ideal_dir + "ideal_fashion_psf.npy")
ideal_mnist = np.load(ideal_dir + "ideal_mnist_psf.npy")

ideal_heuristic_cifar = np.load(ideal_dir + 'ideal_heuristic_cifar_psf.npy')
ideal_heuristic_fashion = np.load(ideal_dir + "ideal_heuristic_fashion_psf.npy")
ideal_heuristic_mnist = np.load(ideal_dir + 'ideal_heuristic_mnist_psf.npy')

psf_list = [ideal_heuristic_mnist, ideal_heuristic_fashion, ideal_heuristic_cifar]
psf_names_list = ['ideal_heuristic_mnist', 'ideal_heuristic_fashion', 'ideal_heuristic_cifar']

# psf_list = [ideal_cifar, ideal_fashion, ideal_mnist]
# psf_names_list = ['ideal_cifar', 'ideal_fashion', 'ideal_mnist']
...
lr_list = [5e-3, 2e-3]
dataset_list = ['mnist', 'fashion_mnist', 'cifar10']

for model_type in model_type_list:
    for lr_value in lr_list:
        for dataset_idx, dataset in enumerate(dataset_list):
            psf_name = psf_names_list[dataset_idx]
            psf = psf_list[dataset_idx]
