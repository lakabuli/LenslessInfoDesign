"""
Building on GaussianPSFLayer with updates from the spectral_diffuser_scope example with full covariance matrices.
...
class RMLPSFLayer(eqx.Module):
    ...
    def __init__(self, object_size: int, num_gaussians: int, psf_size: Tuple[int, int] = (32, 32), measurement_bias: Optional[float] = 0.0, key: Optional[jax.random.PRNGKey] = None):
        ...
        self.covs = jnp.einsum('nij,njk,nkl->nil', R, S, R.transpose(0, 2, 1))
    ...
    def normalize_psf(self):
        ...
        min_std = 0.8  # matching with minimum SD = 0.8
        ...
        new_covs = jnp.einsum("nij,nj,nkj->nik", eigvecs, eigvals, eigvecs)
