...
class LenslessDataGenerator:
    ...
    def load_mnist_data(self):
        ...
        # pad images to be 32 x 32, NOTE that this can change results compared to 28x28 un-padded MI estimates
    ...
    def load_mnist_lb_data(self):
        """Load and preprocess MNIST dataset. Pads each image to be 32 x 32, and scales each MNIST image by an arbitrary seed value.
        ...
        # convert to photons
        x_train = x_train.astype('float32') 
        # multiply by a uniform value that's between 0 and 1
        key = jax.random.PRNGKey(self.seed) 
        scaling_factors = jax.random.uniform(key, shape=(x_train.shape[0],), minval=0.0, maxval=1.0)
        x_train = x_train * scaling_factors[:, None, None]
        # now convert to photon counts
        x_train = x_train / jnp.mean(x_train) 
        x_train = x_train * self.mean_photon_count
        x_test = x_test.astype('float32')
        x_test = x_test / jnp.mean(x_test)
    ...
    def create_dataset(self, x_data, tile_rows=3, tile_cols=3, batch_size=32):
        ...
        dataset = dataset.batch(tile_rows * tile_cols)
    ...
    def create_dataset_untiled(self, x_data, batch_size=32):
        """Creates a TensorFlow dataset that makes zero-padded grayscale images.
        ...
        Returns:
            tf.data.Dataset: Dataset of zero-padded images with shape (N, H, W)
        """
        assert x_data.shape[1] == x_data.shape[2], "Images must be square"
        img_shape = x_data.shape[1]
        x_data = np.pad(x_data, ((0, 0), (16, 16), (16, 16)), 'constant')
